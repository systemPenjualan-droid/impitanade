(function() {
  ...
  function toggleDarkMode(){
    ...
    const circle=toggle.querySelector('.toggle-dark-circle');
    html.classList.toggle('dark');
    toggle.classList.toggle('active');
    if(html.classList.contains('dark')){circle.textContent='☀️';localStorage.setItem('theme','dark');}
    else{circle.textContent='🌙';localStorage.setItem('theme','light');}
  }
  ...
  (function initTheme(){
    ...
    const toggle=document.getElementById('toggleDark');
    const circle=toggle?.querySelector('.toggle-dark-circle');
    if(saved==='dark'){html.classList.add('dark');if(toggle)toggle.classList.add('active');if(circle)circle.textContent='☀️';}
    else{html.classList.remove('dark');if(toggle)toggle.classList.remove('active');if(circle)circle.textContent='🌙';}
  })();
  ...
  function refresh(){
    ...
    if(transaksi.length===0){$('#editKosong').style.display='block';}else{
      ...
      transaksi.forEach((t,i)=>{
        el.innerHTML+=`<div class="edit-item"><div class="edit-item-info"><div class="edit-item-date">${fDate(t.tanggal)}</div><div class="edit-item-nominal">${fRp(t.nominal)} <span style="font-size:10px;color:var(--text-muted)">(${t.hari} hr)</span></div></div><div class="edit-item-actions"><button class="edit-item-btn edit-item-btn--edit edit-btn" data-index="${i}"><i class="bi bi-pencil-fill"></i></button><button class="edit-item-btn edit-item-btn--hapus hapus-btn" data-index="${i}"><i class="bi bi-trash-fill"></i></button></div></div>`;
      });
    }
    ...
    if(transaksi.length===0){$('#tabelKosong').style.display='block';}else{
      ...
      let r=0;
      transaksi.forEach(t=>{r+=t.nominal;tb.innerHTML+=`<tr><td>${fDate(t.tanggal)}</td><td style="font-weight:600">${fRp(t.nominal)}</td><td style="text-align:center">${t.hari}</td><td style="text-align:right;font-weight:700;color:var(--success)">${fRp(r)}</td></tr>`;});
    }
  }
  ...
  $('#btnReset').addEventListener('click',()=>{
    if(confirm('Hapus semua?')){transaksi=[];totalSetor=0;totalHari=0;aktif=false;refresh();switchTab('setup');}
  });
